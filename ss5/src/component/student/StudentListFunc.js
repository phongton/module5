...
function StudentListFunc() {
    ...
    const getAllStudents = async () => {
        ...
        if (startDate && endDate) {
            response = await studentService.getAllStudents(name, startDate, endDate);
        } else {
            response = await studentService.getAllStudents(name);
        }
        setStudents(response);
        if (initialStudents.length === 0) {
            setInitialStudents(response);
        }
    }
                                                ...
                                                <div className="modal-dialog">
                                                    <div className="modal-content">
                                                        <div className="modal-header">
                                                            <h5 className="modal-title" id="deleteModalLabel">Are you sure you want to delete a student named {item.name}?</h5>
